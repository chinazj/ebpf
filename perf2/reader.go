...
func (pr *Reader) Resume() error {
	for i, ring := range pr.rings {
		...
		if err := pr.array.Put(uint32(i), uint32(ring.fd)); err != nil {
			return fmt.Errorf("couldn't put event fd %d for CPU %d: %w", ring.fd, i, err)
		}
	}
	return nil
}
...
func (pr *Reader) ReadInto(rec *Record) error {
	...
	for {
		...
		pr.rings[pr.CPUIndex].loadHead()
		err := pr.readRecordFromRing(rec, pr.rings[pr.CPUIndex])
		pr.CPUIndex = (pr.CPUIndex + 1) % pr.nCPU
		if err == errEOR || len(rec.RawSample) == 0 {
			continue
		}
		return err
	}
...
func (pr *Reader) readRecordFromRing(rec *Record, ring *perfEventRing) error {
	...
	rec.CPU = ring.cpu
	err := readRecord(ring, rec, pr.eventHeader, false)
